...
bool WOLF_catCommandInProgress = false;
...
static void WOLF_i2c_background_task(void *arg) {
	while (1) {
		size_t size = i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, WOLF_inBuff, 1, 100 / portTICK_RATE_MS);
		if (size != 1) {
			continue;
		}
		...
		{
			size = i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, WOLF_inBuff, 1, 100 / portTICK_RATE_MS);
			if (size != 1) {
				continue;
			}

			if (WOLF_inBuff[0] == 0x01) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.CurrentVFO, sizeof(TRX_WOLF.CurrentVFO), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x02) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.VFO_A_Frequency, sizeof(TRX_WOLF.VFO_A_Frequency), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x03) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.VFO_B_Frequency, sizeof(TRX_WOLF.VFO_B_Frequency), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x04) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.VFO_A_Mode, sizeof(TRX_WOLF.VFO_A_Mode), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x05) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.VFO_B_Mode, sizeof(TRX_WOLF.VFO_B_Mode), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x06) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.CAT_Answer, sizeof(TRX_WOLF.CAT_Answer), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x07) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.TX, sizeof(TRX_WOLF.TX), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x08) {
				i2c_slave_read_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)&TRX_WOLF.Tune, sizeof(TRX_WOLF.Tune), 100 / portTICK_RATE_MS);
			}
			continue;
		}
		...
		if (WOLF_inBuff[0] == 0x20) // read has changes? command
		{
			...
			i2c_slave_write_buffer(WOLF_I2C_SLAVE_NUM, WOLF_outBuff, 1, 100 / portTICK_RATE_MS);
			continue;
		}
		...
		if (WOLF_inBuff[0] == 0x30) // read data from interface command
		{
			...
			if (WOLF_inBuff[0] == 0x01) {
				i2c_slave_write_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)TRX_INTERFACE.Message, sizeof(TRX_INTERFACE.Message), 100 / portTICK_RATE_MS);
			}
			if (WOLF_inBuff[0] == 0x02) {
				i2c_slave_write_buffer(WOLF_I2C_SLAVE_NUM, (uint8_t *)TRX_INTERFACE.CAT_Command, sizeof(TRX_INTERFACE.CAT_Command), 100 / portTICK_RATE_MS);
...
void WOLF_setMessage(String message) {
	memset(TRX_INTERFACE.Message, 0x00, sizeof(TRX_WOLF.CAT_Answer));
...
String WOLF_catCommand(String command) {
	if (WOLF_catCommandInProgress) {
		return "Another command in progress";
	}

	WOLF_catCommandInProgress = true;
	...
	while ((millis() - startMs) < 2000) { // CAT Timeout
		if (strlen(TRX_WOLF.CAT_Answer) > 0) {
			WOLF_catCommandInProgress = false;
